...
namespace Com.CodeGame.CodeRacing2015.DevKit.CSharpCgdk
{
    ...
    class iPoint
    {
        ...
        public double[] getStandartPoints(List<Tile> tiles,World world, Game game, Car self)
        {
            double[] points = new double[(tiles.Count-1)*2];
            //points[0, 0] = tiles[0].X*800 + 400;
            //points[0, 1] = tiles[0].Y*800 + 400;
            points[points.Length-2] = tiles[tiles.Count-1].X * 800 + 400;//Last X
            points[points.Length-1] = tiles[tiles.Count - 1].Y * 800 + 400;//Last Y
            for (int i = 1; i < tiles.Count-1; i++)
            {
                ...
                if ((tiles[i - 1].X == tiles[i + 1].X) || (tiles[i - 1].Y == tiles[i + 1].Y))
                {
                    a= new double[2] { (tiles[i].X + 0.5D) * game.TrackTileSize, (tiles[i].Y + 0.5D) * game.TrackTileSize };
